...
trace.set_tracer_provider(TracerProvider(resource = Resource({ResourceAttributes.SERVICE_NAME: "foo1-example"})))
...
def index():
    ...
    parentag = baggage.get_baggage('parentag', ctx) or 1
    ctx = baggage.set_baggage('parentag', 1, ctx)
    context.attach(ctx)

    with tracer.start_as_current_span("foo1") as current_span:

        current_span.set_attribute("parentag.value", parentag)
        ...
        app.logger.debug(f"FOO1 {call_api_02.headers}")
        app.logger.debug(f"FOO1 {call_api_03.headers}")

        session = requests.Session()
        session.send(call_api_02.prepare())
